                                                        ...
                                                        <thead>
                                                            <tr class="delivery-address">
                                                                ...
                                                                <th scope="col"><input type="radio"  name="id" value="{{ $a->id }}" {{ (count($data['address']) == 1 || (isset($a->is_default) && intval($a->is_default)) ? 'checked=checked' : '') }}> {{ $a->type }}
                                                                    @else
                                                                <th scope="col"><input type="radio" disabled name="id" value="{{ $a->id }}"> {{ $a->type }} <em class="address_notice">Address in not selectable.</em><em data-tooltip-address title="{{__('msg.Address is not selectable because selected address is belongs to standard shipping')}}" class="fas fa-info-circle tip"></em>
                                                                    @endif
                                                                    @else
                                                                    @if($a->city_id == '0')
                                                                <th scope="col"><input type="radio"   name="id" value="{{ $a->id }}" {{ (count($data['address']) == 1 || (isset($a->is_default) && intval($a->is_default)) ? 'checked=checked' : '') }}> {{ $a->type }}
                                                                    @else
                                                                <th scope="col"><input type="radio"  disabled name="id" value="{{ $a->id }}"> {{ $a->type }} <em class="address_notice">Address in not selectable.</em><em data-tooltip-address title="{{__('msg.Address is not selectable because selected address is belongs to local shipping')}}" class="fas fa-info-circle tip"></em>
                                                                    @endif
                                                                    @endif
