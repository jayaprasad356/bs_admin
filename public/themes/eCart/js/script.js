        ...
        $(this).toggleClass("active");
                ...
                /*---slide toggle activation---*/
                        ...
                        /*---Category menu---*/
                                ...
                                $(".categories_content_toggle li.has_child .fas").on("click", function () {
                                       
                                if ($(window).width() < 991) {
                                        //let href = $(this).parent("a");
...
$('.track_order_btn').click(function (a) {
    var shipment_id = $(this).closest('.order-bill-slip').find("input[name=shipment_id]").val();
    var order_item_id = $(this).closest('.order-bill-slip').find("input[name=order_item_id]").val();
    var order_status  = $(this).closest('.order-bill-slip').find("input[name=order_status").val();
    
    if(order_status === 'Order not created'){
        alertify.set("notifier", "position", "top-center");
            alertify.warning(order_status);
    }
    else{
    console.log(order_status);
   
    $.ajax({
        url: home + "track_order_view",
        type: "POST",
        dataType: "json",
        data: {
        ...
        success: function (response) {
             $("#loader2").hide();
             console.log(response);
            $(".track_order_view").modal("show");
            var html = '<div class="timeline">';
            $.each(response.activities, function (i, e) {
                html += '<div class="container_modal right"><div class="content"><p>' + msg.order + ' <strong> ' + e.activity + ' </strong> ' + msg.on + ' <strong>' + e.date + '</strong> ';
                if (e.location) {
                    html += msg.from + '<strong> ' + e.location + ' </strong>';
                }
                html += '</p></div></div>';
            });
            html += '</div>';
