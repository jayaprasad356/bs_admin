                        ...
                        (d = Se.event && m.global),
                            ...
                            m.hasContent
                                ...
                                : ((f = m.url.slice(i.length)),
                                  ...
                                  !1 === m.cache && ((i = i.replace(Rt, "$1")), (f = (Et.test(i) ? "&" : "?") + "_=" + Bt++ + f)),
                                  (m.url = i + f)),
                ...
                r(function () {
                    ...
                    function i() {
                        ...
                        var t = g.prop("postdata") ? g.prop("postdata") : {},
                            ...
                            var dtime = (deltime[0]) ? deltime[0]['defaultValue']:'';
                            var deldate = document.getElementsByClassName("deliveryDate");
                            var ddate = (deldate[0]) ? deldate[0]['defaultValue']:'';
                            var wallet_amount = $('[name=wallet_amount]').val()?$('[name=wallet_amount]').val():'0'; //wallet balance add
                            var wallet_used = $('[name=wallet_used]').val()?$('[name=wallet_used]').val():'false';   //payment time if wallet checked
                            var wallet_balance =  $('[name=wallet_balance]').val()?$('[name=wallet_balance]').val():'0'; //payment time wallet_used
        
                        r.ajax({
                            url: home + 'pay-via-ajax',
                            dataType: "json",
                            method: "POST",
                            data: { order: T.order, cart_json: JSON.stringify(t), deliverytime:ddate +' ' +dtime, wallet_amount:wallet_amount, wallet_used:wallet_used, wallet_balance:wallet_balance},
                            xhr: function () {
                            ...
                            function (e) {
                                o.fire({ title: "Error in Communication", text: "Oops! Something wrong, Try again", type: "error", confirmButtonText: "Oh! Ok" }), a();
                            }
                        );
                    }
                    function a() {
                    ...
                    if ((console.log("BUTTON VALUES", JSON.stringify(T)), null != T)) {
                        if (void 0 == T.order || "" == T.order) return void o.fire({ title: "Order Required", text: "Order required to pay", type: "error", confirmButtonText: "Oh! Ok" });
                       // if (void 0 == T.endpoint || "" == T.endpoint) return void o.fire({ title: "Endpoint Required", text: "Endpoint url required to pay", type: "error", confirmButtonText: "Oh! Ok" });
                    }
                    console.log(w),
