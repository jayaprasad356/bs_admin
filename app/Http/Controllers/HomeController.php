...
class HomeController extends Controller {
    ...
    public function faq(Request $request) {
        ...
        $result = $this->post('faq', ['data' => ['get_faqs' => 1]]);

        $this->html('faq', ['title' => __('msg.faq'), 'faq' => $result['data']]);
    ...
    public function product($slug) {
        ...
        $data = ['slug' => $slug];
        $product = $this->post('get-products', ['data' => ['product_slug' => $slug, 'get_all_products' => 1]]);
    
        if (isLoggedIn()) {
            $data[api_param('user_id')] = session()->get('user')['user_id'];
            $product = $this->post('get-products', ['data' => ['product_slug' => $slug, 'get_all_products' => 1, 'user_id' => session()->get('user')['user_id']]]);
        }
    ...
    public function category(Request $request, $slug) {
        ...
        if (Cache::has('categories') && is_array(Cache::get('categories'))) {
            ...
            $subCategory = $this->post('get-sub-categories', ['data' => ['slug' => $slug]]);
            ...
            if (!(isset($subCategory['error']))) {
                ...
                $list = $this->post('get-products', ['data' => ['category_slug' => $slug, 'limit'=>200]]);
               
                $limit = 200;

                $this->html('sub-categories', ['title' => $subCategory['category_name'], 'breadcrumb' => $breadcrumb, 'category' => $breadcrumb, 'list' => $list, 'limit' => $limit, 'category_slug' => $slug]);
            } else {
                ...
                $list = $this->post('get-products', ['data' => ['category_slug' => $slug, 'get_all_products' => 1, 'limit'=>200]]);
                $list = $list['data'] ?? '';

                $limit = 200;

                $this->html('sub-categories', ['title' => $subCategory['category_name'], 'breadcrumb' => $breadcrumb, 'category' => $breadcrumb, 'sub-categories' => $subCategory['data'], 'list' => $list, 'limit' => $limit, 'category_slug' => $slug]);
            }
        } else {
